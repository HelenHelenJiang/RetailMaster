...
#define NCL_FIRMWARE_VERSION_SIZE 8
...
\pre NCL is initialized. The specified Nymi is \ref validation "validated" and has a free \ref commandChannel "command channel".
\post The Nymi's command channel is taken. The Nymi will stop streaming ECG, and then an \ref NCL_EVENT_ECG_STOP will be triggered when the command channel is free.
...
\pre NCL is initialized. The specified Nymi is \ref validation "validated" and has a free \ref commandChannel "command channel".
\post The Nymi's command channel is taken. The Nymi will stop streaming accelerometer data, and then an \ref NCL_EVENT_ACCEL_STOP will be triggered when the command channel is free.
...
\pre NCL is initialized. The specified Nymi is \ref validation "validated" and has a free \ref commandChannel "command channel".
\post The Nymi's command channel is taken. The Nymi will stop streaming gyroscope data, and then an \ref NCL_EVENT_GYRO_STOP will be triggered when the command channel is free.
...
\pre NCL is initialized. The specified Nymi is \ref validation "validated" and has a free \ref commandChannel "command channel".
\post The Nymi's command channel is taken. The Nymi will stop streaming gestures, and then an \ref NCL_EVENT_GESTURE_STOP will be triggered when the command channel is free.
...
\return  \ref NCL_FALSE On failure.
*/
NCL NclBool nclSignAdv(const NclAdv adv, NclPartnerPrivateKey const partnerPrivateKey, NclSig advSig);
